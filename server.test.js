...
describe('Socket.IO — events', () => {
  it('navigate-page persists currentPage/scale and broadcasts to peers', async () => {
    ...
    const raw = store.get(`session:${created.sessionId}`);
    const gameState = typeof raw.gameState === 'string' ? JSON.parse(raw.gameState) : raw.gameState;
    const pdf = gameState.pdfs.find((p) => p.id === 'pdf-1');
  ...
  it('update-layers decompresses the payload, persists it, and re-broadcasts', async () => {
    ...
    const decompressed = pako.inflate(echoed);
    ...
    const raw = store.get(`session:${created.sessionId}`);
    const gameState = typeof raw.gameState === 'string' ? JSON.parse(raw.gameState) : raw.gameState;
    expect(gameState.pageLayers['pdf-X']['2']).toEqual(payload.layers);
  });
